...
def rocketchat_sender(
    ...
    dump = {"alias": alias, "channel": channel, "emoji": ":bell:"}
    ...
    def decorator_sender(func):
        ...
        def wrapper_sender(*args, **kwargs):
            ...
            webhook_url = urljoin(rocketchat_server_url, "/api/v1/chat.postMessage")
            ...
            if master_process:
                contents = [
                    "Your training has **started** :clap: %s"
                    % " ".join(["@" + u for u in user_mentions]),
                    "**Machine name:** %s" % host_name,
                    "**Main call:** %s" % func_name,
                    "**Starting date:** %s" % start_time.strftime(DATE_FORMAT),
                ]
                dump["text"] = "\n".join(contents)
                requests.post(url=webhook_url, data=json.dumps(dump), headers=headers)
            ...
            try:
                ...
                if master_process:
                    ...
                    contents = [
                        "Your training is **complete** :tada: %s"
                        % " ".join(["@" + u for u in user_mentions]),
                        "**Machine name:** %s" % host_name,
                        "**Main call:** %s" % func_name,
                        "**Starting date:** %s" % start_time.strftime(DATE_FORMAT),
                    ...
                    dump["text"] = "\n".join(contents)
                    requests.post(
                        url=webhook_url, data=json.dumps(dump), headers=headers
                    )
                ...
                elapsed_time = end_time - start_time
                contents = [
                    "Your training has **crashed** :skull_crossbones: %s"
                    % " ".join(["@" + u for u in user_mentions]),
                    "**Machine name:** %s" % host_name,
                    "**Main call:** %s" % func_name,
                    "**Starting date:** %s" % start_time.strftime(DATE_FORMAT),
